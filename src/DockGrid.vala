...
public class DockGrid : Gtk.Grid
{
    public DockGrid () {
       ...
       /* Show Desktop Icon */
        ...
        this.attach (icon_size_box, 1, 0, 3, 1);


        this.attach (new LLabel.right (_("Hide Mode:")), 0, 1, 2, 1);
        this.attach (hide_mode_box, 2, 1, 2, 1);


        if (theme_index > 1) {
            this.attach (new LLabel.right (_("Theme:")), 0, 2, 2, 1);
            this.attach (theme_box, 2, 2, 2, 1);
        }


        if (i > 1) {
            this.attach (new LLabel.right (_("Monitor:")), 0, 3, 2, 1);
            this.attach (monitor_box, 2, 3, 2, 1);
        }

        this.attach (new LLabel.right (_("Position:")), 0, 4, 2, 1);
        this.attach (dock_position_box, 2, 4, 2, 1);

        this.attach (new LLabel.right (_("Alignment:")), 0, 5, 2, 1);
        this.attach (dock_alignment_box, 2, 5, 2, 1); 

        this.attach (label_items, 0, 6, 2, 1);
        this.attach (dock_items_box, 2, 6, 2, 1);

        this.attach (new LLabel.right (_("Workspace Overview Icon:")), 0, 7, 2, 1);
        this.attach (overview_icon, 2, 7, 1, 1);

        this.attach (new LLabel.right (_("Show Desktop Icon:")), 0, 8, 2, 1);
        this.attach (desktop_icon, 2, 8, 1, 1);
    }
}
